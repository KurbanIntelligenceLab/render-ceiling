...
def projection_matrices(view_names: list[str],
                        view_map: dict[str, str] | None = None) -> list[np.ndarray]:
    """3x3 ASE rotation matrix per view (screen = pos @ R; cols 0,1 = x,y; col 2 = depth).

    view_map selects the camera-string source; default is the frozen VIEWS. It was previously
    hardcoded, which meant a perturbed-camera condition silently reproduced the frozen ceiling
    instead of measuring its own.
    """
    vm = VIEWS if view_map is None else view_map
    return [rotate(vm[v]) for v in view_names]
...
def reconstruct_positions(
    ...
    Rs = projection_matrices(view_names, view_map)
